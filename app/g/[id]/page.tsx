...
import { ArrowLeft, Plus, CalendarDays, MapPin, Settings, Award } from "lucide-react";
import { Card, Pill, Button, Avatar } from "@/components/ui";
import { MOCK_GROUPS, MOCK_PLANS } from "@/lib/mock";
...
export default async function GroupDetail({
  ...
  const plans = MOCK_PLANS.filter((p) => p.groupName === group.name);
  ...
  return (
    <main className="mx-auto w-full max-w-lg flex-1 px-5 py-6">
      ...
      <Card className="mt-5 p-5">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button className="flex flex-col items-center gap-1">
            <span className="grid h-12 w-12 place-items-center rounded-md border-2 border-dashed border-line text-muted">
              <Plus size={20} />
            </span>
      ...
      {upcoming.length > 0 && (
        <>
          <h2 className="mb-3 mt-7 text-xs font-bold uppercase tracking-wider text-muted">
            Coming up
          </h2>
          <div className="flex flex-col gap-3">
            {upcoming.map((p) => (
              <GroupPlanRow key={p.slug} plan={p} />
            ))}
          </div>
        </>
      )}
...
function GroupPlanRow({ plan }: { plan: (typeof MOCK_PLANS)[number] }) {
  return (
      ...
      <Card hard className="overflow-hidden p-0 transition active:scale-[0.99]">
        <div className="relative">
          ...
          <img
            src={plan.cover}
            alt=""
            className="h-36 w-full border-b-2 border-ink/10 object-cover"
          />
          {plan.adventureNo && (
        ...
        <div className="p-4">
          ...
          <div className="mt-1.5 flex flex-wrap gap-x-3 text-sm text-muted">
            <span className="inline-flex items-center gap-1">
              <CalendarDays size={13} /> {plan.dateLabel}
            </span>
            <span className="inline-flex items-center gap-1">
