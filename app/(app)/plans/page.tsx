...
import {
  CURRENT_USER, MOCK_PLANS, MOCK_INVITES, MOCK_NUDGES, MOCK_OPEN, type PlanSummary,
} from "@/lib/mock";

export default function HomePage() {
  const upcoming = MOCK_PLANS.filter((p) => p.status === "upcoming");
  const next = upcoming[0];
  const rest = upcoming.slice(1);

  return (
    <div>
      <h1 className="font-display text-3xl font-bold">Hey {CURRENT_USER.name}</h1>
      ...
      {next && (
        <div className="mt-5">
          <Countdown
            ...
            cover={next.cover}
            place={next.place}
            targetISO={`${next.date}T16:00:00`}
          />
        </div>
      )}

      {/* for you — inbox */}
      <h2 className="mt-7 mb-3 text-xs font-bold uppercase tracking-wider text-muted">
        For you
      </h2>
          ...
          <Link key={iv.id} href="/invites" className="block">
            <Card className="flex items-center gap-3 p-3 transition active:scale-[0.99]">
              ...
              <div className="min-w-0 flex-1 text-sm">
                ...
                <span className="text-muted">{iv.activity}</span>
              </div>
      ...
      {/* around you — AI / profile suggestions (placeholder feed) */}
      ...
      <p className="mb-3 text-sm text-muted">
        Open plans near you, picked for your interests.
      </p>
      <div className="grid gap-3 sm:grid-cols-2">
        {MOCK_OPEN.map((op) => (
    ...
    <Link href={`/p/${plan.slug}`} className="block">
      <Card hard className="overflow-hidden p-0 transition active:scale-[0.99]">
        <div className="relative">
          ...
          <div className="absolute left-3 top-3">
            <Pill tone="secondary" className="border-2 border-ink/10">{plan.groupName}</Pill>
          </div>
        </div>
        <div className="p-4">
