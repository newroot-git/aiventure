...
import { Card, Button, Avatar, AvatarStack } from "@/components/ui";
import { MOCK_FRIENDS, MOCK_NUDGES, MOCK_GROUPS, sharedGroups } from "@/lib/mock";
...
export default function CrewPage() {
  ...
  return (
    <div>
      ...
      {/* your groups */}
      <Heading
        icon={<Boxes size={20} className="text-secondary" />}
      ...
      {/* friends — info-rich list at the bottom */}
      <Heading
      ...
      <div className="flex flex-col gap-2">
        {MOCK_FRIENDS.map((f) => {
          const shared = sharedGroups(f.id);
          return (
            <Card key={f.id} className="flex items-center gap-3 p-3">
              ...
              <Link href="/nudge"><Button size="sm" variant="soft"><Hand size={14} /></Button></Link>
            </Card>
          );
        })}
      </div>
